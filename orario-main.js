(() => {
  ...
  function applySharedOrariOverrides() {
    ...
    Object.entries(overrides).forEach(([key, value]) => {
      const [direction, race, stopRaw] = key.split("|");
  ...
  function showPage(page) {
    ...
    pageChart.hidden = !chartSelected;
    pageMatrix.hidden = chartSelected;
    ...
    if (!chartSelected && courseCard.open) courseCard.close();
  }
  ...
  function drawOneMatrix(table, rapid) {
    table.querySelector("thead").innerHTML = "<tr><th>Da / a</th>" +
      data.stops.map((stop) => '<th class="text-nowrap">' + stop + "</th>").join("") +
      "</tr>";
    table.querySelector("tbody").innerHTML = data.stops.map((origin, originIndex) => {
    ...
    table.querySelectorAll("tbody td:not(:first-child)").forEach((cell) => {
      cell.addEventListener("mouseenter", () => {
        ...
        row.firstElementChild.classList.add("is-highlighted");
        table.querySelector("thead tr").children[column].classList.add("is-highlighted");
      });
  ...
  function routeText(service) {
    const calls = service.p.map(([stop, minute]) =>
      data.stops[stop] + " " + formatTime(minute)
    );
  ...
  function getSelectedCoincidencePills() {
    if (coincidenceToggle.getAttribute("aria-pressed") !== "true") return [];
  ...
  function openCoincidenceRace(targetRace, targetShift) {
    ...
    if (shiftButton && shiftButton.getAttribute("aria-pressed") !== "true") {
      const savedLeft = chartWrap.scrollLeft;
      const savedTop = chartWrap.scrollTop;
  ...
  function showCourseCard(path, service) {
    ...
    coursePrev.disabled = activeCourseIndex <= 0;
    courseNext.disabled = activeCourseIndex < 0 ||
      activeCourseIndex >= visibleCourseEntries.length - 1;
    ...
    const courseColor = getComputedStyle(path).stroke;
    miniChart.style.setProperty("--course-color", courseColor);
    courseCard.style.setProperty("--course-color", courseColor);
    courseCode.textContent = service.s;
    courseTitle.textContent = service.r;
    courseSummary.textContent = data.stops[first[0]] + " " + formatTime(first[1]) +
    ...
    if (!courseCard.open) courseCard.showModal();
    detail.textContent = routeText(service);
  }
  ...
  function clearStopRouteHighlight() {
    stopAxis.querySelectorAll(".og-axis-label").forEach((label) => {
      ...
      label.querySelectorAll(".og-stop-time").forEach((time) => time.remove());
    });
  }
  ...
  function clearSelectedRoutes() {
    ...
    svg.querySelectorAll(".og-coincidence-layer").forEach((item) => item.remove());
  ...
  function renderCoincidences() {
    svg.querySelectorAll(".og-coincidence-layer").forEach((item) => item.remove());
    if (!chartScales || coincidenceToggle.getAttribute("aria-pressed") !== "true") return;
      ...
      connection.to.forEach(([targetRace, departure]) => {
        ...
        if (!selectedRoutes.has(routeKey(target))) {
          ...
          continuation.forEach(([pointStop, minute]) => {
            ...
            const stopLabel = stopAxis.querySelector(
              '.og-axis-label[data-stop="' + pointStop + '"]'
            );
            if (!stopLabel) return;
    ...
    if (rendered) svg.appendChild(layer);
  }
  ...
  function renderSelectedRoutes() {
    ...
    detail.textContent = selectedRoutes.size + (selectedRoutes.size === 1
      ? " corsa selezionata."
  ...
  function toggleSelectedRoute(path, service, color, adaptView = true) {
    ...
    if (selectedRoutes.size && adaptView) zoomToSelectedRoutes();
      ...
      clearSelectedRoutes();
      detail.textContent = "Selezione rimossa: sono nuovamente visibili tutte le corse.";
      renderSelectedCoursePills();
    }
  }
  ...
  function draw(preserveSelections = false) {
    ...
    const visible = data.services.filter((service) =>
      ...
      (selectedStopFilter === null ||
        service.p.some(([stop]) => stop === selectedStopFilter) ||
    ...
    visibleCourseEntries.sort((a, b) => {
      ...
      return numberA - numberB || a.service.r.localeCompare(b.service.r, "it", {numeric: true});
    });
  ...
  if (mobileMenuToggle) {
    mobileMenuToggle.addEventListener("click", () => {
      const currentlyOpen = isMobileLayout()
        ? root.classList.contains("og-mobile-menu-open")
        : !root.classList.contains("og-turns-hidden");
      setTurniMenuOpen(!currentlyOpen);
    });
  }
  ...
  function syncResidencesHost() {
    if (isMobileLayout()) {
      if (residences.parentElement !== root) {
        root.insertBefore(residences, chartWrap);
      }
      residences.style.transform = "";
      return;
    }
    if (residences.parentElement !== chartWrap) {
