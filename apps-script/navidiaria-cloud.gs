function doGet(e) {
  try {
    ...
    if (action === "directory") {
      ...
      return naviJsonOutput_(
        listNaviPublicDirectory_(sheets.directory)
      );
    }

    return naviJsonOutput_({
      ok: false,
      error: "Azione GET non riconosciuta: " + action
    });
...
function doGet(e) {
  try {
    ...
    if (action === "directory") {
      return naviJsonOutput_(listNaviPublicDirectory_(sheets.directory));
    }
...
function ensureNavidiariaCloudSheets_() {
  ...
  return { users: users, data: data, documents: documents, directory: directory };
}
...
function authNavidiaria_(sheets, request) {
  ...
  updateNaviDirectoryRegistration_(sheets.directory, agentId, true, formatNavidiariaDate_(found.values[3] || now), formatNavidiariaDate_(now));
  return { ok: true, registered: false, mustChangePin: savedHash === NAVIDIARIA_CLOUD_CONFIG.initialPinHash, agent: directoryAgent };
}
  ...
  return {
    ...
    users: rows
      .filter(function(row) {
        ...
        return row[0] && !/^(no|false|0)$/i.test(active);
      })
      .map(function(row) {
        return {
  ...
  if (turnSheet && turnSheet.getLastRow() > 1) {
    ...
    rows.forEach(function(row) {
      ...
      imported[id] = {
        id: id,
        name: name,
        role: id === NAVIDIARIA_CLOUD_CONFIG.adminAgentId ? "admin" : "agent",
