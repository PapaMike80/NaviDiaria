...
function variationsForAgent(data){const activeName=comparableAgentName(activeAgent?.name),variations=new Map();(data?.variazioni_ods||[]).filter(item=>item.attiva!==false&&comparableAgentName(item.agente)===activeName).forEach(item=>{if(item.data&&item.turno_nuovo)variations.set(String(item.data),normalizeScheduleShift(String(item.turno_nuovo).replace(/\*$/,''))) });return variations}
async function getScheduledShift(date){const response=await fetch(`${TURNS_URL}?t=${Date.now()}`);if(!response.ok)throw new Error('NaviTurni non raggiungibile');const data=await response.json(),variations=variationsForAgent(data);let agent=null;Object.values(data.residenze||{}).some(list=>{agent=(list||[]).find(a=>String(a.id)===String(activeAgent?.id));return !!agent});const raw=variations.get(date)??agent?.turni?.[date];if(!raw)throw new Error('Nessun turno assegnato per questa data');return normalizeScheduleShift(raw)}
...
function renderTodaySummary(){
  ...
  $('todaySummary').innerHTML=`<div class="today-date"><span>OGGI</span>${entry.variationFrom?`<em class="variation-badge">VAR. ${escapeHtml(shortShiftLabel(entry.variationFrom))}&gt;${escapeHtml(shortShiftLabel(entry.variationTo||entry.shift))}</em>`:''}${entry.manualModified?`<em class="manual-badge">MOD.${entry.manualFrom&&entry.manualFrom!==entry.manualTo?` ${escapeHtml(shortShiftLabel(entry.manualFrom))}&gt;${escapeHtml(shortShiftLabel(entry.manualTo||entry.shift))}`:''}</em>`:''}<strong>${dateLabel}</strong><em id="todaySaveStatus" class="today-save-status">✓ Salvataggio automatico</em></div><div class="today-inline-form">
    <label class="today-field service-field"><span>Servizio</span><select id="todayShift">${SHIFTS.map(s=>`<option value="${s.code}" ${s.code===entry.shift?'selected':''}>${s.code}</option>`).join('')}</select><em id="todayHours">${minutesToText(shift.hours*60)}</em></label>
    <label class="today-field priority-field"><span>Straordinario</span><div class="minute-input"><input id="todayDelay" type="number" min="0" step="1" value="${Number(entry.delay)||0}"><em>min</em></div></label>
    <label class="today-field priority-field bank-field"><span>Banca ore</span><div class="minute-input"><input id="todayBank" type="number" step="1" value="${Number(entry.bank)||0}"><em>min</em></div></label>
    <label class="today-field inline-check"><span>Rifornimento</span><input id="todayRefuel" type="checkbox" ${entry.refuel?'checked':''}><em id="todayRefuelCredit">${entry.refuel?`+${refuelCredit(entry)} min`:'No'}</em></label>
    <label class="today-field inline-check"><span>Buono pasto</span><input id="todayMeal" type="checkbox" ${entry.mealUsed?'checked':''} ${shift.meal?'':'disabled'}><em id="todayMealStatus">${shift.meal?(entry.mealUsed?'BP USATO':'BP DA ACCREDITARE'):'Non previsto'}</em></label>
    <label class="today-field inline-check"><span>Imbarco</span><input id="todayEmbark" type="checkbox" ${entry.embark?'checked':''}><em id="todayEmbarkStatus">${entry.embark?'Sì':'No'}</em></label>
    <label class="today-field secondary-field"><span>Diaria</span><select id="todayAllowance"><option value="" ${entry.allowanceRate==null?'selected':''}>Nessuna</option>${[0,9,12,24].map(rate=>`<option value="${rate}" ${Number(entry.allowanceRate)===rate?'selected':''}>${rate}%</option>`).join('')}</select></label>
    <div class="today-actions"><button id="restoreTodayInline" class="today-restore-button" type="button">↻ Ripristina da turno</button></div>
  </div>`;
  let saveTimer,statusTimer;const saveTodayChanges=()=>{clearTimeout(saveTimer);const status=$('todaySaveStatus');status.textContent='Salvataggio…';entry.manualFrom=entry.manualFrom||entry.shift;entry.shift=$('todayShift').value;entry.manualTo=entry.shift;entry.manualModified=entry.manualFrom!==entry.manualTo;entry.delay=Number($('todayDelay').value)||0;entry.bank=Number($('todayBank').value)||0;entry.refuel=$('todayRefuel').checked;entry.allowanceRate=$('todayAllowance').value===''?null:Number($('todayAllowance').value);entry.mealUsed=$('todayMeal').checked;entry.embark=$('todayEmbark').checked;entry.imported=false;localStorage.setItem(STORAGE,JSON.stringify(entries));clearTimeout(statusTimer);status.textContent='✓ Salvato';statusTimer=setTimeout(()=>status.textContent='✓ Salvataggio automatico',1400)};const queueSave=()=>{clearTimeout(saveTimer);$('todaySaveStatus').textContent='Salvataggio…';saveTimer=setTimeout(saveTodayChanges,350)};
  $('todayShift').addEventListener('change',()=>{const selected=shiftFor($('todayShift').value);$('todayHours').textContent=minutesToText(selected.hours*60);$('todayAllowance').value=selected.allowance?String(selected.allowanceRate):'';$('todayMeal').disabled=!selected.meal;$('todayMeal').checked=!!selected.meal;$('todayMealStatus').textContent=selected.meal?'BP USATO':'Non previsto';$('todayEmbark').checked=!!selected.embark;updateInlineRefuel();saveTodayChanges()});
  ...
  $('todayRefuel').addEventListener('change',()=>{updateInlineRefuel();saveTodayChanges()});
  $('todayMeal').addEventListener('change',()=>{$('todayMealStatus').textContent=$('todayMeal').checked?'BP USATO':'BP DA ACCREDITARE';saveTodayChanges()});$('todayEmbark').addEventListener('change',()=>{$('todayEmbarkStatus').textContent=$('todayEmbark').checked?'Sì':'No';saveTodayChanges()});$('todayAllowance').addEventListener('change',saveTodayChanges);['todayDelay','todayBank'].forEach(id=>{$(id).addEventListener('input',queueSave);$(id).addEventListener('change',saveTodayChanges)});
  $('restoreTodayInline').addEventListener('click',async()=>{const button=$('restoreTodayInline');button.disabled=true;try{const code=await getScheduledShift(todayIso()),competence=shiftFor(code);$('todayShift').value=code;$('todayHours').textContent=minutesToText(competence.hours*60);$('todayAllowance').value=competence.allowance?String(competence.allowanceRate):'';$('todayMeal').disabled=!competence.meal;$('todayMeal').checked=!!competence.meal;$('todayMealStatus').textContent=competence.meal?'BP USATO':'Non previsto';$('todayEmbark').checked=!!competence.embark;updateInlineRefuel();saveTodayChanges();notify(`Ripristinato il turno ${code}`)}catch(error){notify(error.message)}finally{button.disabled=false}});
}
