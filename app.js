...
function saveInlineEntry(entry,field,value){
  if(field==='shift'){const original=entry.manualFrom||entry.shift;entry.shift=value;entry.manualFrom=original;entry.manualTo=value;entry.manualModified=original!==value;const competence=shiftFor(value);entry.embark=!!competence.embark;entry.mealUsed=!!competence.meal;entry.allowanceRate=competence.allowance?competence.allowanceRate:null}
  else if(field==='delay')entry.delay=Math.max(0,Number(value)||0);else if(field==='bank')entry.bank=Number(value)||0;else if(field==='mealUsed')entry.mealUsed=value==='true';else if(field==='embark')entry.embark=value==='true';else if(field==='allowanceRate')entry.allowanceRate=value===''?null:Number(value);
  entry.imported=false;localStorage.setItem(STORAGE,JSON.stringify(entries));render();notify('Giornata aggiornata')
}
function startInlineEdit(cell){
  ...
  else{editor=document.createElement('select');const options=field==='mealUsed'?[['true','USATO'],['false','DA ACCREDITARE']]:field==='embark'?[['true','Sì'],['false','No']]:[['','Nessuna'],['0','0%'],['9','9%'],['12','12%'],['24','24%']];options.forEach(([value,label])=>editor.add(new Option(label,value,false,String(field==='allowanceRate'?(entry.allowanceRate??''):entry[field])===value)))}
  editor.className='inline-cell-editor';cell.replaceChildren(editor);editor.focus();if(editor.select)editor.select();let saved=false;const commit=async()=>{if(saved)return;saved=true;let value=editor.value;if(field==='shift'&&value==='__restore__'){editor.disabled=true;try{value=await getScheduledShift(entry.date)}catch(error){notify(error.message);render();return}}saveInlineEntry(entry,field,value)};editor.addEventListener('change',commit);editor.addEventListener('blur',commit);editor.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();commit()}if(event.key==='Escape'){saved=true;render()}})
}
