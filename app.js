...
function renderTodaySummary(){
  ...
  $('saveTodayInline').addEventListener('click',()=>{entry.manualFrom=entry.manualFrom||entry.shift;entry.shift=$('todayShift').value;entry.manualTo=entry.shift;entry.delay=Number($('todayDelay').value)||0;entry.refuel=$('todayRefuel').checked;entry.allowanceRate=$('todayAllowance').value===''?null:Number($('todayAllowance').value);entry.mealUsed=$('todayMeal').checked;entry.embark=$('todayEmbark').checked;entry.imported=false;entry.manualModified=true;persist();notify('Giornata di oggi aggiornata')});
}
